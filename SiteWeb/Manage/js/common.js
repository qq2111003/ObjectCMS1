...
var Message = {
    ...
    alert: function (str, type, title) {
        $.messager.alert(title || '提示', str, type);
    },
...
var UIDialog = {
    ...
    Init: function (id, title, width, height, modal) {
        UIDialog.id = id;
        if (!$('#UIWindowBox').length) {
            $('body').append('<div id="UIWindowBox"></div>');
        }
        $('#UIWindowBox').append('<div id="' + id + '" class="easyui-window" minimizable="false" closed="true" closable="true" modal="true" title="' + title + '" style="width:' + width + 'px;height:' + height + 'px;">' +
                            '<div class="easyui-layout" fit="true">' +
                            '   <div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;">' +
                            '       <iframe width="100%" height="100%" id="UIDialog_' + id + '_Iframe" name="UIDialog_' + id + '_Iframe" frameborder="0"></iframe>' +
                            '   </div>' +
                            '   <div id="UIWinBtnBox_' + id + '" region="south" border="false" style="text-align:right;padding:0;">' +
                            '   </div>' +
                            '</div>' +
                            '</div>');
        return this;
    ...
    CreateBtn: function (type, text, fn) {
        $('#UIWinBtnBox_' + UIDialog.id).append('<a id="UIBtn' + type + '" class="easyui-linkbutton" data-options="plain:true,iconCls:\'icon-' + type + '\'" href="javascript:void(0)">' + text + '</a>');
    ...
    Open: function (url) {
        ...
        $('#UIDialog_' + UIDialog.id + '_Iframe').attr('src', url);
        return this;
    },
    Close: function (fn) {
        if (fn) {
...
function OpenWindow(title, url, width, height) {
    UIDialog.Init('new_window', title, width, height);
    UIDialog.CreateBtn('ok', '保存', function () {
        if (!$(GetFrame('UIDialog_new_window_Iframe')).find('input[type=submit]').length) {
            var inputelement = document.createElement('input');
            inputelement.type = "submit";
            inputelement.style.display = 'none';
            $(GetFrame('UIDialog_new_window_Iframe')).find('form').append(inputelement);
        }
        $(GetFrame('UIDialog_new_window_Iframe')).find('form input[type=submit]').click();
    });
...
var py = {
    ...
    getSinglePinyin: function (strChar) {
        for (pinyin in this.dictPinyin) {
            if (py.dictPinyin[pinyin].indexOf(strChar) !== -1) {
                return pinyin;
            }
        }
        if (/[\u0020-\u007e]/.test(strChar)) {
    ...
    getPinyin: function (strText) {
        var iLength = strText.length, strChar, strRet = '';
        for (var i = 0; i < iLength; i++) {
            strChar = strText.substr(i, 1);
            strRet += this.getSinglePinyin(strChar);
        }
        return strRet;
    }
}
