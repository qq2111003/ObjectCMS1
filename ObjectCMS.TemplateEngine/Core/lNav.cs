...
namespace XHR.Template
{
    public class lNav
    {
        ...
        public static string ReplaceLabeltoData(string labelHTML, Hashtable[] param_arr)
        {
            ...
            int Top = 0;
            string sql = "1=1";
            if (ParamController.GetParam("sql", param_arr) != null)
            {
                sql = ParamController.GetParam("sql", param_arr);
            }
            ...
            dv.RowFilter = "IsDeleted=0 and HasFrontChannel=1 and "+sql+"";
            ...
            for (int i = 0; i < dv.Count; i++)
            {
                ...
                if (xml.IsExistXml(xmlFilePath) && !string.IsNullOrEmpty(xmlFilePath))
                {
                    ...
                    {
                        dr["Title"] = Entitle;
                        sb.Append(lModelData.ReplaceLabeltoData(labelHTML.buqufendaxiaoxieReplace("{index}", i + "").buqufendaxiaoxieReplace("{recordcount}", ds.Tables[0].Rows.Count.ToString()), dr, param_arr));
                        if (Top != 0 && (i + 1) == Top)
                        {
                            break;
                        }
