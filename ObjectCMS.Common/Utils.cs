...
namespace ObjectCMS.Common
{
    ...
    public class Utils
    {
        public const string ASSEMBLY_VERSION = "3.5.0";
        ...
        public static string CutString(string s, int MaxLength)
        {
            ...
            int tmpLen = 0;
            for (Int32 i = 0; i < stringChar.Length; i++)
            {
                tmpLen += encoding.GetByteCount(stringChar, i, 1);
                if (tmpLen <= MaxLength)
                {
        ...
        public static string HtmlEecode2(string str)
        {
            var s = "";
        ...
        {
            if (!Directory.Exists(Utils.GetMapPath(name)))
                Utils.MakeSureDirectoryPathExists(Utils.GetMapPath(name));
        }
        ...
        public static string GetStringByte(string s, int length)
        {
        ...
        public static string AddQueryToUrl(string oriUrl, string key, string value)
        {
            if (oriUrl.Contains("?"))
            {
                if (oriUrl.Contains("&" + key) || oriUrl.Contains("?" + key))
                {
                    Regex reg = new Regex(@"&*" + key + @"=[^&]+", RegexOptions.IgnoreCase);
                    oriUrl = reg.Replace(oriUrl, "");
                }
                if (oriUrl.Split('?')[1] == "")
                {
                    oriUrl += key + "=" + value;
        ...
        public static string RemoveQueryFromUrl(string oriUrl, string key)
        {
